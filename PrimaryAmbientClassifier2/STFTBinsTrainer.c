...
void LLPAMSProcessNPR(STFTBinsTrainer *PMA)
{
	...
	float magDiff = (magnitudeSum < FLT_EPSILON) ? 0.0f : (magnitudeRight - magnitudeLeft) / magnitudeSum;
	float phaseDiff = fabs(atan2f(0.0f, rR) - atan2f(0.0f, lR));
	trainerAdd2Vector(PMA);
	PMA->trainingDataOut1[PMA->trainingDataCount - 1][0] = magDiff;
	PMA->trainingDataOut1[PMA->trainingDataCount - 1][1] = phaseDiff;
	...
	for (i = 1; i < PMA->zeroPaddedHalfLength; i++)
	{
		...
		float magnitudeLeft = sqrtf(lR * lR + lI * lI);
		float magnitudeRight = sqrtf(rR * rR + rI * rI);
		float magnitudeSum = magnitudeLeft + magnitudeRight;
		float magDiff = (magnitudeSum < FLT_EPSILON) ? 0.0f : (magnitudeRight - magnitudeLeft) / magnitudeSum;
		float phaseDiff = fabs(atan2f(rI, rR) - atan2f(lI, lR));
		trainerAdd2Vector(PMA);
		PMA->trainingDataOut1[PMA->trainingDataCount - 1][0] = magDiff;
		PMA->trainingDataOut1[PMA->trainingDataCount - 1][1] = phaseDiff;
	}
